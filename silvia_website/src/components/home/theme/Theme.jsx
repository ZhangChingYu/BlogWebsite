...
import LifeShow from "../../../lottie/Life Simple.json";
import LifeType from "../../../lottie/life theme-2.json";
...
const Theme = () => {
    ...
    const [lifeEnd, setLifeEnd] = useState(false);
    return( 
        <div id="theme" className="theme">
            ...
            <div className="theme_cat" title="check out my life" onClick={()=>{navigator("/life")}} onPointerEnter={()=>{setCatHover(true)}} onPointerLeave={()=>{setCatHover(false)}}>
                ...
                <div className="theme_animate">
                    {lifeEnd?
                        <Lottie animationData={LifeType} loop={true} autoPlay={false}/>:
                        <Lottie animationData={LifeShow} loop={false} autoPlay={false} onComplete={()=>{setLifeEnd(true)}}/>
                    }
                </div>
                <div className="theme_cat_btn"> 
                    <p>My Life</p>
                </div>
                <Link to={'/work'} className="theme_dragon_link">
                    <motion.div className="theme_dragon_circle" animate={{x:"-5rem", boxShadow:"4px 0px 0px 0px rgba(0, 0, 0, 0.25)"}} transition={{duration:1, type:"tween"}} initial={{x:"-10rem", boxShadow:"0px 4px 4px 0px rgba(0, 0, 0, 0.25)"}}>
                    </motion.div>
                </Link>
